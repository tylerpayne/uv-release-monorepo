...
Use `uvr version --bump post` to enter the post-release cycle, then `uvr release` to publish:
...
Post-releases are made from the original release tag, not from main. **Take care merging post-release branches back to main** as pyproject.toml versions will conflict since the branch is based on an old tag.
